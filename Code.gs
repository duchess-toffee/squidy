function onHomepage(e) {
	const cardHeader = CardService.newCardHeader()
		...
		.setImageUrl(
			'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/160/twitter/71/squid_1f991.png'
		);

	const aboutSquidy = CardService.newCardSection()
		.addWidget(
			CardService.newTextParagraph().setText(
				'Squidy allows you to take personal notes for your calendar events.'
			)
		)
		.addWidget(
	...
	const aboutSquidy2 = CardService.newCardSection()
		.addWidget(
			CardService.newTextParagraph().setText(
				'You can also send emails from your event notes!'
			)
		)
		.addWidget(
			CardService.newImage()
	...
	const aboutSquidy3 = CardService.newCardSection()
		.addWidget(
			CardService.newTextParagraph().setText(
				'Go on! Click on an event to get started with Squidy!'
			)
		)
		.addWidget(
			CardService.newImage()
...
function renderEventUI(eventId, eventNotes, event) {
	const hasEventTitle =
		CalendarApp.getEventById(eventId) == null
			? 'New Event'
			: CalendarApp.getEventById(eventId).getTitle();
	const hasEventNotes = eventNotes !== null ? eventNotes : '';

	let card = CardService.newCardBuilder().setHeader(
		CardService.newCardHeader().setTitle(hasEventTitle)
	);
	...
	card
		.addSection(CardService.newCardSection().addWidget(notesContent))
...
function renderEmailUI(event) {
	...
	if (event.calendar.hasOwnProperty('attendees')) {
		event.calendar.attendees.forEach((attendee, index) =>
			eventAttendees.push(attendee.email)
		);
	}
	const hasEventAttendees =
		eventAttendees.length > 0 ? eventAttendees.toString() : '';
	const hasEventContent = event.formInput.hasOwnProperty('event_notes')
		? event.formInput.event_notes
		: '';
	...
	const emailButton = CardService.newTextButton()
		.setText('Send Email')
		.setOnClickAction(CardService.newAction().setFunctionName('sendEmail'));
...
function sendDraft(event) {
	GmailApp.createDraft(
		event.formInput.email_recipients,
		event.formInput.email_subject,
		event.formInput.email_content
	);
}
